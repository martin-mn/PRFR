#!/bin/bash
# Completion of a dw2 array, established per task from sacct.  Never from
# squeue -u: that output spans every project submitting under this user.
#   bash verify.sh {probe8|probe9|vor8|vor9}
#
...
case "$1" in
  *8) RUN=run_1e8 ;;
  *9) RUN=run_1e9 ;;
  *) echo "usage: bash verify.sh {probe8|probe9|vor8|vor9}"; exit 1 ;;
esac
