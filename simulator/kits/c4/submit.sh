#!/bin/bash
# Submit one c4 array: stage the game list, capture the array id with --parsable into JOBID_<tag>, and append one
# line to the cross-project ledger ~/jobs.tsv.   bash submit.sh {probe_e4|prod_e4}
# cube corner N = 1000, beta = 3, u = 1e-4 (the large population with the small one's selection and mutation), eps = 1e-4.  The run length is compiled in (compile.sh 1e7 -> sf_1e7.x); the run directory is run_1e7/.
set -e
L=1e7
case "$1" in
  probe_e4) SCRIPT=probe_e4.slurm; TAG=probe_e4; NT=8; IDX="1368 2892 2895 2898 2901 1902 2436 2664";
      NOTE="c4 probe, cube corner N = 1000, beta = 3, u = 1e-4 (the large population with the small one's selection and mutation), eps = 1e-4, itend=1e7, ie=3 (e4); 8 cells: innermost, farthest per quadrant, 3 PD/W cells near the donation ray at r=3,6,9" ;;
  prod_e4)  SCRIPT=prod_e4.slurm;  TAG=prod_e4;  NT=512; IDX="$(seq 1368 3 2901)";
      NOTE="c4 512-point sunflower, cube corner N = 1000, beta = 3, u = 1e-4 (the large population with the small one's selection and mutation), eps = 1e-4, itend=1e7, ie=3 (e4), 10 reps (1368-2901:3)" ;;
...
echo "$jid" > "JOBID_$TAG"
printf '%s\tPartnersRivals\tc4_%s\t%s\t%s\t%s\n' "$(date +%F)" "$TAG" "$jid" "$NT" "$NOTE" >> ~/jobs.tsv
