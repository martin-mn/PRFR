#!/usr/bin/env python3
"""
SI Figure 10: the small-population memory-two run of SI Figure 4 (N = 100, beta = 3, mu = 1e-4) at three
error rates, eps = 1e-3, 1e-4, 1e-5.

    python3 SIFigure10/sifig10.py        ->  SIFigure10/SIFig10.pdf and SIFig10.png

The sheet has the layout and colours of main text Figure 4 (common.wfsheet.draw): one row per error rate, the middle
row being SI Figure 4's own run (f2_e4), the other two the runs el_e3 and el_e5; columns efficiency, most abundant atom,
...
Data: data/robustness/games_{f2_e4,el_e3,el_e5}.csv and m2_atom_sizes.csv (data/robustness/wfruns.py).  The code is
the original FinalFigures/sifig9.py (which drew SI Figures 9 and 10; SIFigure9/sifig9.py is its other half) with the
