#!/bin/bash
# Submit one eh array: stage the game list, capture the array id with --parsable into JOBID_<tag>, and append one
# line to the cross-project ledger ~/jobs.tsv.   bash submit.sh {probe_e5|prod_e5|probe_e3|prod_e3}
# error-rate series at N = 1000, beta = 100, u = 1e-2 (Figure 4's large population), eps = 1e-5 and 1e-3.  The run length is compiled in (compile.sh 1e7 -> sf_1e7.x); the run directory is run_1e7/.
set -e
L=1e7
case "$1" in
  probe_e5) SCRIPT=probe_e5.slurm; TAG=probe_e5; NT=8; IDX="1366 2890 2893 2896 2899 1900 2434 2662";
      NOTE="eh probe, error-rate series at N = 1000, beta = 100, u = 1e-2 (Figure 4's large population), eps = 1e-5 and 1e-3, itend=1e7, ie=1 (e5); 8 cells: innermost, farthest per quadrant, 3 PD/W cells near the donation ray at r=3,6,9" ;;
  prod_e5)  SCRIPT=prod_e5.slurm;  TAG=prod_e5;  NT=512; IDX="$(seq 1366 3 2899)";
      NOTE="eh 512-point sunflower, error-rate series at N = 1000, beta = 100, u = 1e-2 (Figure 4's large population), eps = 1e-5 and 1e-3, itend=1e7, ie=1 (e5), 10 reps (1366-2899:3)" ;;
  probe_e3) SCRIPT=probe_e3.slurm; TAG=probe_e3; NT=8; IDX="1367 2891 2894 2897 2900 1901 2435 2663";
      NOTE="eh probe, error-rate series at N = 1000, beta = 100, u = 1e-2 (Figure 4's large population), eps = 1e-5 and 1e-3, itend=1e7, ie=2 (e3); 8 cells: innermost, farthest per quadrant, 3 PD/W cells near the donation ray at r=3,6,9" ;;
  prod_e3)  SCRIPT=prod_e3.slurm;  TAG=prod_e3;  NT=512; IDX="$(seq 1367 3 2900)";
      NOTE="eh 512-point sunflower, error-rate series at N = 1000, beta = 100, u = 1e-2 (Figure 4's large population), eps = 1e-5 and 1e-3, itend=1e7, ie=2 (e3), 10 reps (1367-2900:3)" ;;
...
echo "$jid" > "JOBID_$TAG"
printf '%s\tPartnersRivals\teh_%s\t%s\t%s\t%s\n' "$(date +%F)" "$TAG" "$jid" "$NT" "$NOTE" >> ~/jobs.tsv
