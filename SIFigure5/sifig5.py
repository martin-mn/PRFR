...
SI Figure 5: main text Figure 5 for the small population.
    ...
    python3 sifig5.py           ->  SIFig5.pdf / SIFig5.png  (here)

Six disks: the two Wright-Fisher runs at N = 100, beta = 3, mu = 1e-4, eps = 1e-4 (memory one above memory two,
data/runs/m1_N100.csv and m2_N100.csv); the columns the three properties, 1** efficient (the atoms 100, 110,
...
path; the drawing is unchanged.  The original drew Figure 5 and this figure in one run; main text Figure 5
(N = 1000) is Figure5/fig5.py, with the same draw().
"""
import os
import sys
...
if __name__ == "__main__":
    draw([k for k in wfdata.ROWS if k[1] == "F2"], os.path.join(HERE, "SIFig5.pdf"))   # SI Figure 5: the small population
