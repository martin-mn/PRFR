#!/bin/bash
# Completion of an lm array, established per task from sacct -- never from squeue -u, whose output spans every
# project submitting under this user.    bash verify.sh {probe|prod} {g0|g01|g05}
...
set -e
TAG=$1; G=$2; [ -n "$G" ] || { echo "usage: bash verify.sh {probe|prod} {g0|g01|g05}"; exit 1; }
RUN=run_$G
J="JOBID_${TAG}_$G"; [ -f "$J" ] || { echo "$J not found -- this kit has no record of that array"; exit 1; }
jid=$(cat "$J")
echo "array $jid"
...
while read -r isl; do
  ...
  itf=""; [ -f "$RUN/h$isl" ] && itf=$(awk '/^# run:/{for(i=1;i<NF;i++) if($i=="itend") print $(i+1); exit}' "$RUN/h$isl")
  if [ -n "$itf" ] && [ "$itf" != 10000000 ]; then nshort=$((nshort+1)); miss="$miss $isl(itend $itf)"; continue; fi
