#!/bin/bash
# Submit one dw4 array: stage the game list, capture the array id with --parsable into
# JOBID_<tag>, and append one line to the cross-project ledger ~/jobs.tsv.
#   bash submit.sh probe7   8 tasks: the rate + convergence probe
#   bash submit.sh prod7    512 tasks: the sunflower
...
case "$1" in
  probe7) L=1e7; SCRIPT=probe_1e7.slurm; TAG=probe7; NT=8; IDX="1368 2892 2895 2898 2901 1902 2436 2664";
          NOTE="dw4 probe, N = 1000, beta = 3, u = 1e-2: weak selection, frequent mutation, eps=1e-4, itend=1e7; 8 cells: innermost, farthest per quadrant, 3 PD/W cells near the donation ray at r=3,6,10" ;;
  prod7)  L=1e7; SCRIPT=prod_1e7.slurm;  TAG=vor7;   NT=512; IDX="$(seq 1368 3 2901)";
          NOTE="dw4 512-point sunflower, N = 1000, beta = 3, u = 1e-2: weak selection, frequent mutation, eps=1e-4, itend=1e7, 10 reps (1368-2901:3)" ;;
  *) echo "usage: bash submit.sh {probe7|prod7}"; exit 1 ;;
esac
...
echo "$jid" > "JOBID_$TAG"
printf '%s\tPartnersRivals\tdw4_%s\t%s\t%s\t%s\n' "$(date +%F)" "$TAG" "$jid" "$NT" "$NOTE" >> ~/jobs.tsv
